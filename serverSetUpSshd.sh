...
cat <<ENDCAT > /tmp/serverSetUpSshd.sh.sh;
...
echo "Clean up"
rm /home/phil/buildTools.zip /home/phil/platformTools.zip /tmp/serverSetUpSshd.sh.sh
...
rsync -e "ssh -o ForwardX11=no" /tmp/serverSetUpSshd.sh.sh root@$1:serverSetUpSshd.sh;                        # Copy server set up file                      
